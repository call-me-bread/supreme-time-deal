...
class TimeLimit(
	...
	companion object  {
		fun create(startTime: LocalDateTime, endTime: LocalDateTime): TimeLimit {
			return TimeLimit(
				startTime = startTime,
				endTime =  endTime
			)
	...
	fun plusStartTime(time: Long, type: ChronoUnit) {
		this.startTime = this.startTime.plus(time ,type)
	}

	fun plusEndTime(time: Long, type: ChronoUnit) {
